...
- 代码块支持语言选择与用户端语法高亮。
- 代码块顶部提供语言选择、语言搜索、自动换行和复制操作；代码正文左侧展示按原始代码行计算的行号。语言选择更新结构化节点属性，自动换行只影响当前编辑会话。
...
- 代码块展示语言标识、语法高亮、行号和复制操作；行号与管理端编辑态使用同一生成规则。
...
- 行号是展示层能力，不写入文档 JSON，也不进入复制文本；自动换行后仍按原始逻辑行编号，空行同样保留编号。
...
23. 管理端与用户端代码块均显示连续行号；行号不参与复制和持久化，自动换行不改变行号含义。
...
- 代码块行号在管理端与用户端均按代码原始逻辑行展示；行号不影响编辑内容、复制内容和文档 JSON。
