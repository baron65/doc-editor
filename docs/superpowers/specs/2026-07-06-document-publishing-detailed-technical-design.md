...
> 日期：2026-07-06
> 状态：已完成文档一致性复核；待真实仓库 Sprint 0 验证
...
- 不新增顶层 Maven 模块或独立微服务，代码进入当前承载业务需求的既有 Spring Boot 服务。
- 不为本模块新增 Redis、Kafka、Elasticsearch、WebSocket 或 Node 后端服务。
- 对象存储优先通过现有 `platform-support-storage-v2` 接入，由本模块的适配器隔离 SDK 类型。
- 管理端与用户端在同一 Umi 工程内使用四个独立路由，管理端编辑器依赖不能进入普通用户首屏包。
- 权限继续由现有菜单、后台接口权限和登录体系负责，本模块不建立文档级 ACL。
...
以下是逻辑结构，根包名与现有业务模块保持一致：

```text
documentcenter/
  controller/
    admin/
      AdminDocumentTreeController.java
      AdminDocumentController.java
      AdminDocumentAssetController.java
    user/
      UserDocumentController.java
      UserDocumentAssetController.java
  web/
    DocumentSaveRequestHttpMessageConverter.java
  application/
    DocumentTreeCommandService.java
  ...
  domain/
    DocumentContentValidator.java
    DocumentNameNormalizer.java
    DocumentStateCalculator.java
    DocumentTreePolicy.java
    DocumentAssetPolicy.java
    model/
      DocumentContent.java
      ContentValidationResult.java
      DocumentPublishState.java
  infrastructure/
    entity/
      DocNodeEntity.java
      DocDocumentEntity.java
      DocAssetEntity.java
      DocAssetRefEntity.java
      DocTreeMetaEntity.java
    mapper/
      DocNodeMapper.java
      DocDocumentMapper.java
      DocAssetMapper.java
      DocAssetRefMapper.java
      DocTreeMetaMapper.java
...
- Controller 只做鉴权入口、Bean Validation、DTO 转换和响应封装；草稿保存 DTO 由专用 Converter 在进入 Controller 前受限解析。
- Application Service 编排事务与 Mapper，不把事务散落在 Controller。
- Domain 类负责无 IO 的名称、树规则、内容和状态校验。
- Infrastructure 隔离 MyBatis、数据库和平台存储 SDK。
- 管理端 DTO 与用户端 DTO 完全分开；禁止从同一实体自动映射后再清空草稿字段。
...
```java
public interface DocumentTreeCommandService {
...
以下路径为模块逻辑路径，实际网关前缀、版本前缀和管理端前缀服从现有系统规范。示例使用：

```text
管理端：/document-center/admin
用户端：/document-center
```

外层响应复用 `platform-support-web-v2`，不在本模块新建第二套 envelope。本文用以下逻辑结构表达字段；接入时只替换外层，不改变 `data`：
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/document-center/admin/tree` | 获取全部目录、文档和 `treeRevision` |
| POST | `/document-center/admin/directories` | 新建目录 |
| POST | `/document-center/admin/documents` | 新建草稿文档 |
| PATCH | `/document-center/admin/directories/{directoryId}` | 重命名目录 |
| PATCH | `/document-center/admin/nodes/{nodeId}/position` | 移动或排序节点 |
| DELETE | `/document-center/admin/nodes/{nodeId}` | 删除空目录或未发布文档 |
| GET | `/document-center/admin/documents/{documentId}` | 获取编辑稿详情 |
| PUT | `/document-center/admin/documents/{documentId}/draft` | 条件保存编辑稿 |
| POST | `/document-center/admin/documents/{documentId}/publish` | 发布当前编辑稿 |
| POST | `/document-center/admin/documents/{documentId}/unpublish` | 携带 expectedPublicationVersion 取消发布 |
| POST | `/document-center/admin/documents/{documentId}/assets` | 上传单个图片或附件 |
| GET | `/document-center/admin/documents/{documentId}/assets/{assetId}` | 管理端读取资源 |

### 8.2 获取管理端树

```http
GET /document-center/admin/tree
```
...
```http
POST /document-center/admin/directories
...
```http
POST /document-center/admin/documents
```
...
```http
PATCH /document-center/admin/directories/19001
```
...
```http
PATCH /document-center/admin/nodes/19002/position
```
...
```http
DELETE /document-center/admin/nodes/19002?expectedTreeRevision=21
```
...
```http
GET /document-center/admin/documents/19002
```
...
```http
PUT /document-center/admin/documents/19002/draft
```
...
```http
POST /document-center/admin/documents/19002/publish
```
...
```http
POST /document-center/admin/documents/19002/unpublish
```
...
```http
POST /document-center/admin/documents/19002/assets
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/document-center/tree` | 获取已发布文档树和默认文档 |
| GET | `/document-center/documents/{documentId}` | 获取线上稿详情 |
| GET | `/document-center/search?q=` | 按线上标题搜索 |
| GET | `/document-center/documents/{documentId}/assets/{assetId}` | 读取已发布资源 |
...
```http
GET /document-center/documents/19002
...
```http
GET /document-center/search?q=%E6%8E%A5%E5%85%A5&limit=20
```
...
```http
GET /document-center/documents/19002/assets/19100
```
...
- 全局异常处理器负责 envelope 和日志，本模块只抛平台业务异常与错误码。
...
正式开发前用真实 Umi Max 4.5.3、TypeScript 4.9.5 工程验证：
...
2. 确认平台 ID 生成、审计字段、权限注解、统一 envelope 与异常映射。
...
以下项目不改变本文架构，但拿到真实业务模块后必须用现有实现替换占位名称：

| 替换点 | 本文假设 | 实施动作 |
|---|---|---|
| 根包与 Maven 模块 | 放入既有业务服务 | 按真实分包和 Mapper 扫描位置调整 |
| API envelope | 逻辑 `code/message/data/requestId` | 复用 `platform-support-web-v2` 实际类型 |
| 权限 | 现有后台权限与系统登录 | 替换为实际注解、权限码和菜单配置 |
| ID | 现有 BIGINT 生成器 | 确认前端字符串序列化策略 |
| 审计字段 | BIGINT 用户 ID + DATETIME | 继承实际审计基类与用户上下文 |
| 数据库 | MySQL/OceanBase MySQL 模式兼容 DDL | 用实际版本验证索引、事务和字符集 |
| 存储 | `platform-support-storage-v2` 可流式上传/读取/删除 | 实现 `PlatformStorageV2Adapter` 并做故障测试 |
| 前端 UI | 现有 Tree、Modal、Drawer、Upload | 用实际设计系统组件实现交互契约 |
| 测试 | JUnit/前端测试工具未知 | 确认真实命令、CI 报告与 E2E 基础设施 |
| 配置 | Nacos/项目配置绑定 | 替换前缀并补充配置变更流程 |

这些是接入信息，不应重新打开已经确认的产品范围。若现有平台能力与本文假设冲突，只调整适配层、字段命名或外围协议，不应削弱双快照、revision、资源隔离和管理/用户查询分离四条核心边界。

### 23.1 开发就绪门禁

当前文档已达到 Sprint 0 标准，但在以下产物完成前，不进入完整业务编码：

1. 在真实后端模块输出有效 dependency tree，确认 Mapper 扫描、事务管理器、Jackson 收敛和测试插件。
2. 在目标数据库验证五表 DDL、二进制排序规则、唯一索引多 NULL、事务和 `INSERT ... SELECT`。
3. 用真实 `platform-support-storage-v2` 完成流式上传、读取、删除、超时和对象不存在的最小验证。
4. 确认统一 envelope、HTTP 状态、权限注解、CSRF、ID 和审计字段的实际接入方式。
5. 确定会真实执行后端、前端和 E2E 测试的命令，并验证测试报告数量非零。
6. 在真实 Umi Max 4.5.3 / TypeScript 4.9.5 工程完成 Tiptap、Markdown、Static Renderer、Mermaid 和构建拆包 Spike，锁定精确依赖版本。

Sprint 0 完成后可并行进入三条开发线：后端表与领域服务、前端 Schema/编辑器、用户端 Reader/页面壳；存储适配器依赖第 3 项验证结果。
