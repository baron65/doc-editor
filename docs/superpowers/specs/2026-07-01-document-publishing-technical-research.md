...
- 状态：已按父 POM 与明确前端技术栈完成校准；无需继续分析无关后端子模块
...
建议先按 UTF-8 字节数限制原始内容，再使用项目统一的 Jackson 2.15.2 `ObjectMapper` 解析为 `JsonNode`，递归校验并返回带 JSON Pointer 的错误位置。不要把前端 TypeScript 类型当作安全边界，也不要使用 POM 中同时存在的 Fastjson 1.x 解析这类不可信富文本内容。
...
任何一步失败都回滚。存储对象已在事务前处于 READY，不需要远程调用。
...
| POST | `/admin/document-center/assets/{id}/complete` | 直传对象存储时确认资源 READY |
...
| 阶段 | 工作 | 粗略工作量 |
|---|---|---|
| 0 | 目标模块依赖检查、Umi/Tailwind 配置审计、测试门禁、Tiptap/Lexical Spike | 2～4 人日 |
...
9. Mermaid 错误是否只需在官方管理端阻止，还是要求所有 API 调用都由服务端权威校验；后者需要 Node 校验服务或等价运行时。
...
3. **内部存储 SDK 预上传 + DRAFT/PUBLISHED 资源引用**，让发布事务保持纯数据库原子操作。
