...
5. 后端在现有 Maven 多模块工程中增加文档领域包；MyBatis-Plus 负责简单单表 CRUD，自定义 Mapper XML 负责 revision 条件更新、发布、资源引用切换和树重排，不引入 JPA。
6. 数据库保存“编辑稿 / 线上稿”双快照和递增的 `draft_revision`。自动保存使用条件更新检测冲突。
7. 发布动作在一个 Spring 数据库事务中完成：校验编辑稿版本、复制标题与 JSON、替换线上资源引用、记录发布人和时间。
8. **对象存储不参与发布事务。** 优先复用项目管理的 `platform-support-storage-v2`，图片和附件先上传并在数据库标记为 `READY`，发布事务只切换数据库引用。
9. 对象存储保持私有；内容 JSON 只保存 `assetId`，不保存永久文件 URL。读取资源前由后端校验其是否属于当前线上稿，再通过现有存储能力返回短期地址或代理文件流。
...
3. POM 没有直接管理 AWS S3 SDK，却管理了内部存储 SDK。后端应先验证并复用 `platform-support-storage-v2`，不能先假定需要新增 AWS SDK。
...
网络层复用 Umi Max 已有 request 插件及 `src/app.tsx` 中的统一拦截器、错误转换和后端 envelope 配置，不再创建独立 Axios 实例。自动保存使用普通 `request` 服务函数加自有串行队列；不要直接依赖 `useRequest` 的刷新行为表达 revision 状态机。读取详情和搜索可以取消陈旧请求，但已经发出的保存请求不能通过前端取消来假定服务端没有落库。[Umi Max Request](https://umijs.org/docs/max/request)
...
| DocumentAssetService | 内部存储 SDK 适配、状态、授权读取、清理 |
| DocumentContentValidator | JSON Schema 白名单、大小、URL、节点和资源校验 |
| MyBatis-Plus Mappers | 简单 CRUD 使用 BaseMapper；关键路径使用显式 XML SQL 与结果映射 |

发布应是一个清晰的 Command Service，不要把发布逻辑散落在 Controller 和多个 Mapper 调用点。

用户已确认其他子模块与本需求无关，因此不新增顶层 Maven module，也不扩展无关 SDK。Controller、Service、Mapper、DTO 和存储适配器直接放入当前承载该需求的既有业务服务模块，并遵循其现有分包规范。
...
底层存储接入有三条可行路径；当前 MVP 最终选择“平台存储 SDK + Spring Boot 代理上传”，预签名直传只作为未来替换方案：

**复用 `platform-support-storage-v2`（首选）**
...
- 统一异常继续复用 `platform-support-web-v2` 的响应 envelope 和异常处理约定；Spring Boot 2.3 没有 `ProblemDetail`。
...
路径前缀应服从现有系统规范。当前详细设计已将模块逻辑路径固定如下；网关统一前缀仍由现有系统补充。

### 10.1 管理端 API

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/document-center/admin/tree` | 全量树、状态、treeRevision |
| POST | `/document-center/admin/directories` | 新建目录 |
| POST | `/document-center/admin/documents` | 新建草稿文档 |
| PATCH | `/document-center/admin/nodes/{id}/position` | 移动/排序，携带 expectedTreeRevision |
| PATCH | `/document-center/admin/directories/{id}` | 目录重命名，即时生效 |
| DELETE | `/document-center/admin/nodes/{id}` | 删除空目录或未发布文档 |
| GET | `/document-center/admin/documents/{id}` | 编辑稿、线上摘要、draftRevision |
| PUT | `/document-center/admin/documents/{id}/draft` | 保存标题、JSON与 expectedDraftRevision；后端从正文提取资源引用 |
| POST | `/document-center/admin/documents/{id}/publish` | 携带 expectedDraftRevision 原子发布 |
| POST | `/document-center/admin/documents/{id}/unpublish` | 携带 expectedPublicationVersion 取消发布 |
| POST | `/document-center/admin/documents/{id}/assets` | Spring Boot 代理的单文件流式上传 |
| GET | `/document-center/admin/documents/{documentId}/assets/{assetId}` | 管理端草稿资源访问 |
...
| GET | `/document-center/documents/{documentId}/assets/{assetId}` | 校验 PUBLISHED 引用后下载或跳转短签名 URL |
...
错误响应复用 `platform-support-web-v2` 的现有 envelope 与 `@ControllerAdvice` 约定。Spring Boot 2.3 不提供 `ProblemDetail`，不得为本模块引入 Spring Framework 6 API 或第二套错误协议。
...
在没有真实仓库配置与内部 SDK 接口的情况下只能给出区间，以下按已有鉴权、内部存储、数据库迁移和基础 UI 组件估算：

| 阶段 | 工作 | 粗略工作量 |
|---|---|---|
| 0 | 目标模块依赖检查、Umi/Tailwind 配置审计、测试门禁、Tiptap/Lexical 与 Markdown 粘贴 Spike | 2～4 人日 |
...
合计暂维持约 29～46 人日。前后端并行且内部存储、鉴权、审计和测试门禁已经成熟时，通常是 3～5 周日历时间；如果 `platform-support-storage-v2` 不支持所需上传/临时访问能力，或父 POM 的测试与依赖收敛需要专项治理，工作量会增加。

## 18. 真实仓库接入前必须确认

1. 目标服务模块的 MyBatis-Plus/PageHelper 拦截器配置、Mapper 扫描和常规 dependency tree；无需提供其他无关子模块。
2. 数据库服务端类型、版本、OceanBase 租户模式、字符集/排序规则、JSON/CLOB 支持和迁移工具。
3. `platform-support-storage-v2` 的上传、下载、临时 URL、删除、元数据与文件扫描接口。
4. `platform-support-web-v2` 的统一响应、异常、鉴权注解和操作审计规范。
5. Surefire/CI 如何覆盖 `skipTests=true`，当前实际使用 JUnit 4 还是 JUnit 5。
6. 当前 ID 生成器、审计基类、时间字段和 MapStruct MapperConfig 约定。
7. React 精确版本、包管理器/lockfile、Umi 路由与 request 配置、Tailwind content/Preflight 配置和前端测试命令。
8. CSP 与反向代理的上传大小、超时和响应头限制。
